...
There is no project-specific build pipeline. Use lightweight local checks instead:

- `rg --files -g '*.md'` lists all documentation files.
- `python -m http.server 8000` serves the repo for browser-based Markdown preview if your editor does not provide one.

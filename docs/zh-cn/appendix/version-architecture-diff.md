...
| 文件 | 责任 |
|---|---|
| `scripts/upstream-baseline.json` | 记录稳定版本、main SHA、已审计源码目标、关键模块和命令/旧模式断言 |
| `scripts/check_upstream_drift.py` | 扫描发布文档，验证固定链接、关键模块引用、稳定版命令和已修复模式 |
...
报告包含两条基线、扫描文件数、固定源码链接数、命令断言计数和具体发现。任一链接使用分支/标签、指向未审计路径，关键模块引用消失，旧目录或禁用命令回归，或者 `TOOLS.md` 被重新描述成自动 Bootstrap 文件，命令都会以非零状态退出。

!!! note "离线检查能证明什么"

    检查器不会访问 GitHub、模型或 Channel。它验证的是“当前文档是否仍符合上次人工确认过的 SHA/路径清单”，不能替代下一次升级时对新上游提交的人工审计。更新 `audited_sources` 前，维护者必须在上游 clone 中确认目标对象存在并重新检查行为差异。

完整升级步骤、关键模块审查面和 PR 验收命令记录在仓库根目录的 `CONTRIBUTING.md`。

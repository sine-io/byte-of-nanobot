...
因为一旦你把 Telegram、Discord 之类的平台逻辑直接塞进 Agent 主循环里，后面每加一个入口，核心推理代码就要跟着长一层分支。`MessageBus` 的价值不在于“多一个概念”，而在于把 **消息从哪里来** 和 **消息怎么被处理** 彻底拆开。

你可以把这一章理解成：不是为了接 Telegram 才学总线，而是为了让 Agent 核心以后**不再依赖任何具体平台**。
...
nanobot 的解法是**消息总线（MessageBus）**：用两个异步队列把 Agent 和 I/O 解耦。下面保留这条主线，但省略进度事件、流式增量、重试、配对和运行时控制，因此它是**简化教学模型**，不是可以替换 nanobot 生产实现的完整副本。
...
对应 v0.2.2 的 [`InboundMessage` / `OutboundMessage`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/bus/events.py)：
...
对应 v0.2.2 的 [`MessageBus`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/bus/queue.py)。教学版只保留入站、出站两个队列和四个读写方法：
...
主干就是两个 `asyncio.Queue`：一个进，一个出。真实实现还暴露队列长度，并由其他总线模块承载进度和运行时事件；不要从示例规模推断完整系统规模。
...
对应 v0.2.2 的 [`BaseChannel`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/channels/base.py)。下面只保留启动、停止、发送和入站转发接口：
...
!!! info "main 差异：Channel 已改为自包含插件包"
    v0.2.2 的操作基线仍是内置的单文件适配器；不要把下面的源码导航变化混进稳定版安装步骤。固定对照的 `main@b189a376` 已把实现拆进包式 [`telegram/runtime.py`](https://github.com/HKUDS/nanobot/blob/b189a37648e4fa64f662b15de4f78ffd0bab403b/nanobot/channels/telegram/runtime.py)、[`discord/runtime.py`](https://github.com/HKUDS/nanobot/blob/b189a37648e4fa64f662b15de4f78ffd0bab403b/nanobot/channels/discord/runtime.py) 和 [`slack/runtime.py`](https://github.com/HKUDS/nanobot/blob/b189a37648e4fa64f662b15de4f78ffd0bab403b/nanobot/channels/slack/runtime.py)，并用各包的 `manifest.py` 声明插件元数据。

```python
class TelegramChannel(BaseChannel):
    """Telegram Channel——只演示统一接口的教学简化版。"""
    name = "telegram"
...
真实 v0.2.2 中，[`AgentLoop`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/agent/loop.py)负责消费消息、会话与出站编排，而模型流式输出和工具迭代由 [`AgentRunner`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/agent/runner.py)执行。下面为了延续前章代码，把两者压在一个教学类中。
...
真实 nanobot 由固定版本的 [`ChannelManager`](https://github.com/HKUDS/nanobot/blob/e2e75c913f3524d4bc5b23487a4eed5329eef182/nanobot/channels/manager.py)发现已启用的 Channel、管理生命周期，并集中处理出站路由与重试。这里用独立函数把路由原则摊开。
...
| 概念 | 我们的代码 | nanobot |
|------|-----------|---------|
| MessageBus | 2 个 asyncio.Queue | 双队列主干相同，另有队列状态、进度和运行时事件协作 |
| InboundMessage | 4 个字段 | 同 + media, metadata, session_key_override |
| BaseChannel | 3 个抽象方法 | 同 + ACL/配对、音频、流式进度和推理展示契约 |
| AgentLoop.run | 消费消息 → 内嵌工具循环 → 发回 | AgentLoop 编排状态与 I/O，AgentRunner 执行模型/工具循环 |
| 消息路由 | 单独的 `route_outbound` | 集中在 ChannelManager，并包含发送重试与生命周期管理 |
...
- 对应代码快照：[examples/hero/ch04-mini-agent-gateway.py](../examples/hero/ch04-mini-agent-gateway.py)
